...
public:
    ...
    TreeNode*buildtree(vector<int>&postorder,int poststart,int postend,vector<int>&inorder,int instart,int inend,map<int,int>&mp)
    {
        ...
        int lpos=inroot-instart;
        root->left=buildtree(postorder,poststart,poststart+lpos-1,inorder,instart,inroot-1,mp);
        root->right=buildtree(postorder,poststart+lpos,postend-1,inorder,inroot+1,inend,mp);
        return root;
    }
};